...
const Hero = () => {
    const { currentTheme, isLoaded, weatherInfo, selectedDay, setSelectedDay } = useWeather()
    
    if (!weatherInfo) return null;

    const displayData = weatherInfo.forecast[selectedDay];
           ...
           <div className="w-full justify-center md:justify-start flex">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center text-center md:text-left gap-8">
                <div className="space-y-4">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 backdrop-blur-md text-sm font-medium">
                        ...
                        {selectedDay === 'today' ? 'Live Updates' : `${selectedDay.charAt(0).toUpperCase() + selectedDay.slice(1)}'s Overview`}
                    </div>
                    <h2 className="text-6xl md:text-7xl font-black tracking-tighter drop-shadow-2xl">
                        {displayData.temp}<span className="text-5xl align-top mt-4 inline-block">°C</span>  
                    </h2>
                    <div className="flex items-center justify-center md:justify-start gap-4">
                        <span className="text-lg e font-medium opacity-90">{selectedDay === 'today' ? `Feels like ${weatherInfo.current.feelsLike}°C` : `${selectedDay.charAt(0).toUpperCase() + selectedDay.slice(1)} Average`}</span>
                        <div className="h-6 w-px bg-white/30"></div>
                        <span className="text-lg opacity-70">H: {displayData.high}° L: {displayData.low}°</span>
                    </div>
                </div>
                <div className="flex flex-col items-center justify-end">
                    {getWeatherIcon(displayData.theme, 24, "w-20 h-20 animate-bounce")}
                    <div className="mt-4 px-6 py-2 rounded-2xl bg-white/10 border border-white/10 text-sm font-medium backdrop-blur-sm">
                        Mostly {displayData.theme}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-12">
                {[
                    ...
                    <div key={i} className="flex items-center gap-3 bg-white/5 p-4 rounded-2xl hover:bg-white/10 transition-all cursor-default border border-transparent hover:border-white/10">
